...
class GitUpCheck:
	...
	def _getData(self, store):
		...
		if store == REDISNAME:
			...
			result = list(map(lambda x: str(x).split(':'), self._getFromRedis()))
...
def main(results):
	...
	if path != None and remotepath != None and results.store != None:
		checker.addItem(path, remotepath, store=results.store, addr=results.store)
...
def parsing(parse):
	...
	parse.add_argument("--addr", help="Append address host:port to remote store")
